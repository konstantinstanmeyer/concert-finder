...
export default async function artistEvents(req: NextApiRequest, res: NextApiResponse){
    ...
    console.log("YESYSY")
    const response = await axios.get(process.env.API_URL + `events.json?${search ? `keyword=${search.indexOf(' ') >= 0 ? search.split(' ').join('+') : search}&` : ""}${zipcode ? `postalCode=${zipcode}&` : ""}${city ? `city=${city}&` : ""}${stateCode ? `stateCode=${stateCode}&` : ""}${genres ? reduceGenres(genres) : ""}classificationName=Music&includeSpellcheck=yes&page=${page}&size=${size}&apikey=${process.env.API_KEY}`)
